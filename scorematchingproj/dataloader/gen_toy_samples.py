...
def gen_8gaussians(n_samples, scale=4):
...
def gen_checkerboard(n_samples):
...
def gen_rings(n_samples):
    ...
    x = (
        torch.stack(
            [torch.cat([circ4_x, circ3_x, circ2_x, circ1_x]), torch.cat([circ4_y, circ3_y, circ2_y, circ1_y])], dim=1
        )
        * 3.0
    )
